...
.PHONY: test
test:
	docker-compose run --rm  app  sh -c "python manage.py wait_for_db && python manage.py makemigrations  && python manage.py migrate && python manage.py test && flake8"

.PHONY: erd-dot
erd-dot:
	docker-compose run --rm  app  sh -c  "python manage.py graph_models -a > erd.dot"